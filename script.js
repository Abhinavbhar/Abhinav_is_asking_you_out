...
// Call handleNoMouseOver for the first 10 seconds
timerId = setTimeout(() => {
  handleNoMouseOver(); // Call the function
}, 10000); // 10000 milliseconds = 10 seconds

// Stop handleNoMouseOver after 10 seconds
setTimeout(() => {
  clearTimeout(timerId);
  noBtn.removeEventListener("mouseover", handleNoMouseOver); // Remove the event listener
  console.log('handleNoMouseOver stopped after 10 seconds');
}, 10000);
const createCenteredImage = () => {
  const imageUrl = 'https://indianmemetemplates.com/wp-content/uploads/thukra-ke-mera-pyaar-mera-intkam-dekhegi.jpg';
  const audioUrl = 'thukrake.mp3';

  // Create and append the image
  const centeredImage = document.createElement('img');
  centeredImage.src = imageUrl;
  centeredImage.alt = 'Centered Image';
  centeredImage.style.position = 'relative';
  centeredImage.style.top = '10%';
  centeredImage.style.right = '10%';
  centeredImage.style.transform = 'scale(0.5)'; // Resize the image to 50% of its original size
  ...
  retryBtn.addEventListener('click', () => {
    // Remove the image and stop the audio
...
noBtn.addEventListener("click", createCenteredImage);
